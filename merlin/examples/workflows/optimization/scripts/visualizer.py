...
ax.plot_surface(X_mesh[0], X_mesh[1], Z_mesh, rstride = 5, cstride = 5, cmap = 'jet', alpha = .4, edgecolor = 'none')
...
ax.plot_surface(X_mesh[0], X_mesh[1], Z_pred, rstride = 5, cstride = 5, cmap = 'jet', alpha = .4, edgecolor = 'none')
ax.scatter(existing_X[:, 0], existing_X[:, 1], existing_y, marker="x")
ax.view_init(45, 45)
ax.set_xlabel('DIM_1')
ax.set_ylabel('DIM_2')
...
ax.plot_surface(X_mesh[0], X_mesh[1], np.clip(Z_mesh, -100 ,100), rstride = 5, cstride = 5, cmap = 'jet', alpha = .4, edgecolor = 'none')
...
ax.plot_surface(X_mesh[0], X_mesh[1], np.clip(Z_pred, -100 ,100), rstride = 5, cstride = 5, cmap = 'jet', alpha = .4, edgecolor = 'none')
ax.scatter(existing_X[:, 0], existing_X[:, 1], np.clip(existing_y, -100 ,100), marker="x")
ax.view_init(45, 45)
ax.set_xlabel('DIM_1')
ax.set_ylabel('DIM_2')
...
ax.annotate("Predicted Optimum", xy=optimum, xytext=(1,0),
            arrowprops=dict(width=0.01))
ax.annotate("Current Best", xy=old_best, xytext=(-1.5,1.5),
            arrowprops=dict(width=0.01))
ax.annotate("Actual Minimum", xy=(1, 1), xytext=(1.5,2.5),
            arrowprops=dict(width=0.01))
...
ax.scatter(new_explore_star_samples[:, 0], new_explore_star_samples[:, 1], label='Explore_star Samples')

...
class RichWorkerFormatter(WorkerFormatter):
    ...
    def format_and_display(
        ...
        Args:
            ...
            console (Optional[Console]): Rich console instance to 
                render output. If None, a new console is created.
        """
        if console is None:
            console = Console()
        ...
        if layout_config.use_compact_view:
            self._display_compact_view(logical_workers, merlin_db, filters, stats, console)
            return
        
        # Display summary panels
        if layout_config.show_summary_panels:
            self._display_summary_panels(stats, filters, layout_config, console)
            console.print()  # Empty line
    ...
    def _display_compact_view(
        ...
        Args:
            logical_workers (List[db_scripts.entities.logical_worker_entity.LogicalWorkerEntity]):
                List of logical worker objects to display.
            merlin_db (db_scripts.merlin_db.MerlinDatabase): Reference to the Merlin database for
                querying data.
            filters (Dict): Active filters applied to the display.
            stats (Dict[str, int]): Aggregated worker statistics (e.g., running counts).
            console (Console): Rich console instance used for output.
        """
    ...
    def _display_summary_panels(
        ...
        Args:
            ...
            console (Console): 
                Rich Console object used to render the panels.
        """
    ...
    def _sort_physical_workers(self, data: List[Dict]) -> List[Dict]:
        ...
        return sorted(data, key=lambda row: (
            0 if row['_sort_status'] == "RUNNING" else 1,
            row['worker'],
            row['instance']
        ))

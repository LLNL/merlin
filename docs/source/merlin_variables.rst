...
Similarly, the ``$(VLAUNCHER)`` variable will behave in the same way as the ``$(LAUNCHER)``
variable. The difference is that instead of reading the nodes, procs, cores per task, and gpus
defined in the ``run`` portion of a step, it will read specific shell variables. These shell
variables will automatically be generated by Merlin if you provide the ``$(VLAUNCHER)`` variable
in a step cmd but can be modified by the user. The current shell variables that are created are:
...
The ``$(VLAUNCHER)`` variable would be substituted to ``srun -N 5 -n 3``.

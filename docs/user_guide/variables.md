...
Reserved variables are study variables that Merlin uses. They may be referenced within a spec file but not reassigned or overridden.
...
| <pre><code>$(WORKSPACE)</code></pre> | The workspace directory for a single step. | <pre><code>$(OUTPUT_PATH)/ensemble_name_$(MERLIN_TIMESTAMP)/step_name/</code></pre> |

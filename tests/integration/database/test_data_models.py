...
class TestModelInteractions:
    ...
    def test_logical_physical_worker_relationship(self):
        ...
        physical_worker1 = PhysicalWorkerModel(
            id="pw-1", 
            logical_worker_id=logical_worker.id,
            name="celery@worker1.host1"
        )
        
        physical_worker2 = PhysicalWorkerModel(
            id="pw-2", 
            logical_worker_id=logical_worker.id,
            name="celery@worker1.host2"
        )
        
        # Update logical worker with physical worker IDs
        logical_worker.update_fields({
            "physical_workers": [physical_worker1.id, physical_worker2.id]
        })

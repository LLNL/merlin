...
class TestBaseDataModel:
    ...
    def test_dump_to_json_file(self, mocker: MockerFixture, concrete_base_model_class: BaseDataModel, db_scripts_testing_dir: FixtureStr):
        """
...
class TestRunModel:
    ...
    def test_fields_allowed_to_be_updated(self):
        """Test that fields_allowed_to_be_updated returns expected values."""
        run = RunModel()
        assert set(run.fields_allowed_to_be_updated) == {
            "parent", "child", "run_complete", "additional_data", "workers"
        }
...
class TestPhysicalWorkerModel:
    ...
    def test_fields_allowed_to_be_updated(self):
        ...
        expected_fields = [
            "launch_cmd", "args", "pid", "status", "heartbeat_timestamp", 
            "latest_start_time", "restart_count"
        ]

...
def worker_config() -> WorkerConfig:
    """
    Fixture that provides a basic WorkerConfig instance.

    Returns:
        A WorkerConfig object with default settings.
    """
    return WorkerConfig(
        name="test_worker",
        queues={"queue1", "queue2"},
        nodes=1,
        env={"OUTPUT_PATH": workers_testing_dir},
    )
...
def test_constructor_sets_fields_and_calls_db_create(worker_config: WorkerConfig, mock_db: MagicMock):
    ...
    This test verifies that:
        - The worker configuration is stored in the `worker_config` attribute.
        - The MerlinDatabase.create method is called with the correct arguments.

    Args:
        worker_config: A WorkerConfig instance for initializing the CeleryWorker.
        mock_db: A mocked MerlinDatabase to prevent real database interaction.
    """
    worker = CeleryWorker(worker_config)
...
def test_verify_args_adds_name_and_logging_flags(mocker: MockerFixture, celery_worker: CeleryWorker, mock_db: MagicMock):
    ...
    Args:
        mocker: Pytest mocker fixture.
        celery_worker: A CeleryWorker object for testing.
        mock_db: Mocked MerlinDatabase to avoid real database writes.
    """
    celery_worker.batch_manager = MagicMock()
    celery_worker.batch_manager.is_parallel.return_value = True
    mock_logger = mocker.patch("merlin.workers.celery_worker.LOG")

    celery_worker._verify_args()

    assert "-n test_worker" in celery_worker.worker_config.args
    assert "-l" in celery_worker.worker_config.args
...
def test_get_launch_command_returns_expanded_command(mocker: MockerFixture, celery_worker: CeleryWorker, mock_db: MagicMock):
    ...
    """
    celery_worker.batch_manager = MagicMock()
    celery_worker.batch_manager.create_worker_launch_command.return_value = "celery -A ..."

    cmd = celery_worker.get_launch_command("--override", disable_logs=True)

    assert isinstance(cmd, str)
    assert "celery" in cmd


def test_should_launch_rejects_if_machine_check_fails(mocker: MockerFixture, worker_config: WorkerConfig, mock_db: MagicMock):
    """
...
def test_should_launch_rejects_if_output_path_missing(mocker: MockerFixture, worker_config: WorkerConfig, mock_db: MagicMock):
    """
    ...
    """
    worker_config.machines = ["host1"]
    worker_config.env = {"OUTPUT_PATH": "/nonexistent"}
...
def test_should_launch_rejects_due_to_running_queues(mocker: MockerFixture, celery_worker: CeleryWorker, mock_db: MagicMock):
    ...
    result = celery_worker.should_launch()
...
def test_launch_worker_runs_if_should_launch(mocker: MockerFixture, celery_worker: CeleryWorker, mock_db: MagicMock):
    ...
    Args:
        ...
        mock_db: Fixture for the Merlin database mock.
    """
    mocker.patch.object(celery_worker, "should_launch", return_value=True)
    mocker.patch.object(celery_worker, "get_launch_command", return_value="echo hello")
...
def test_launch_worker_raises_if_popen_fails(mocker: MockerFixture, celery_worker: CeleryWorker, mock_db: MagicMock):
    ...
    """
    mocker.patch.object(celery_worker, "should_launch", return_value=True)
    mocker.patch.object(celery_worker, "get_launch_command", return_value="fail")
...
def test_get_metadata_returns_expected_dict(celery_worker: CeleryWorker, mock_db: MagicMock):
    ...
    Args:
        ...
        mock_db: Mocked MerlinDatabase object.
    """
    metadata = celery_worker.get_metadata()

    assert WorkerConfig.from_dict(metadata) == celery_worker.worker_config

...
class TestResponsiveLayoutManager:
    ...
    @pytest.mark.parametrize("width, expected_size", [
    ...
    @pytest.mark.parametrize("status, expected_icon, expected_text", [
        (WorkerStatus.RUNNING, "✓", "RUNNING"),
        (WorkerStatus.STOPPED, "✗", "STOPPED"),
        (WorkerStatus.STALLED, "⚠", "STALLED"),
        (WorkerStatus.REBOOTING, "↻", "REBOOTING"),
    ])
    def test_format_status(self, layout_manager: ResponsiveLayoutManager, status: WorkerStatus, expected_icon: str, expected_text: str):
        """
...
class TestRichWorkerFormatter:
    ...
    @pytest.mark.parametrize("status, expected_icon, expected_color", [
    ...
    def test_format_status(self, formatter: RichWorkerFormatter, status: Union[WorkerStatus, str], expected_icon: str, expected_color: str):
        """
    ...
    @pytest.mark.parametrize("status, timestamp, expected", [
        (WorkerStatus.RUNNING, datetime.now() - timedelta(hours=1, minutes=30), "1h 30m"),
        (WorkerStatus.RUNNING, None, "-"),
    ])
    def test_format_uptime_or_downtime_running_worker(self, formatter: RichWorkerFormatter, status: WorkerStatus, timestamp: timedelta, expected: str):
        """
    ...
    def test_format_uptime_or_downtime_stopped_worker(self, formatter: RichWorkerFormatter, status: WorkerStatus, timestamp: timedelta, expected: str):
        """
        Test uptime or downtime formatting.
    ...
    def test_display_summary_panels_vertical(
        self,
        mocker: MockerFixture,
        formatter: RichWorkerFormatter
    ):
